...
run_test tractor sample.yaml -x "//data/servers/item" --expect 2 -m "sequence items become item elements"
run_test tractor sample.yaml -x "//data/servers/item[name='web-1']" --expect 1 -m "sequence items with nested mappings are queryable"
run_test tractor sample.yaml -x "//data/servers/item[name='web-1']/port[.='8080']" --expect 1 -m "can query nested values within sequence items"
run_test tractor sample.yaml -x "//data/features/item" --expect 3 -m "scalar sequences become item elements"
run_test tractor sample.yaml -x "//data/features/item[.='auth']" --expect 1 -m "scalar sequence items have text content"
...
run_test tractor sample.yaml -x "//data/flow_list/item" --expect 3 -m "flow sequences become item elements"
