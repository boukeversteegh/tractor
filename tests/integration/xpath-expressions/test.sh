...
# let expressions
run_test tractor test -s "let x = 1; let y = 2;" -l typescript -x "let \$v := //variable return \$v/name" -o value --expect 2 -m "let expression returns bound results"

# for expressions
run_test tractor test -s "let x = 1; let y = 2;" -l typescript -x "for \$v in //name return string(\$v)" -o value --expect 2 -m "for expression iterates over matches"

# if expressions
run_test tractor test -s "let x = 1;" -l typescript -x "if (//variable) then //name else ()" -o value --expect 1 -m "if expression with true branch"
run_test tractor test -s "let x = 1;" -l typescript -x "if (//function) then //name else //variable" -o value --expect 1 -m "if expression with false branch"
...
run_test tractor test -s "let x = 1;" -l typescript -x "let \$v := //name return \$v" -o value --expect 1 -m "let with variable reference in return"
