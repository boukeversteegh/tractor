...
use crate::format::{ViewField, GroupDimension, Projection, render_report};
...
pub fn run_query(args: QueryArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Compute schema when requested via -p schema or -v schema.
    if ctx.projection == Projection::Schema || ctx.view.has(ViewField::Schema) {
