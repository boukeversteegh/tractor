...
pub struct Report {
    ...
    /// Opaque text rendering of the match schema (set when `-v schema` or
    /// `-p schema` is requested). Lives at `/schema` in the report shape.
    /// Kept as a string rather than a structured node — the design defers
