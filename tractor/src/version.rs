...
pub fn print_version() {
    ...
    let max_len = DEPENDENCY_VERSIONS
        .iter()
        .map(|(n, _)| n.len())
        .max()
        .unwrap_or(0);
...
pub fn print_version_verbose() {
    ...
    // Find max name length for alignment
    let max_dep_len = DEPENDENCY_VERSIONS
        .iter()
        .map(|(n, _)| n.len())
        .max()
        .unwrap_or(0);
    ...
    let max_lang_len = GRAMMAR_VERSIONS
        .iter()
        .map(|(crate_name, _, lang_name)| {
            let crate_suffix = crate_name
    ...
    let max_version_len = GRAMMAR_VERSIONS
        .iter()
        .map(|(_, v, _)| v.len())
        .max()
        .unwrap_or(0);
