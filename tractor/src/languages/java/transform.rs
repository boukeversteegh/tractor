//! Java transform logic — thin dispatcher driven by `rules::rule`.
//!
...
///      from the original tree-sitter kind), look it up in `JavaKind`,
...
pub fn transform(xot: &mut Xot, node: XotNode) -> Result<TransformAction, xot::Error> {
    ...
    // `map_element_name` returned `None`.
    let kind = match JavaKind::from_str(&kind_str) {
        Some(k) => k,
