//! TypeScript / JavaScript transform logic — thin dispatcher driven
//! by `rules::rule`.
...
/// Transform a TypeScript / JavaScript AST node.
///
/// Dispatch is split in two:
///   1. If the node carries a `kind` attribute, look it up in
///      `TsKind`, fetch its `Rule` from `rules::rule`, and execute
...
pub fn transform(xot: &mut Xot, node: XotNode) -> Result<TransformAction, xot::Error> {
    ...
    let kind = match TsKind::from_str(&kind_str) {
        Some(k) => k,
