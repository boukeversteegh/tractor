//! PHP transform logic — thin dispatcher driven by `rules::rule`.
//!
...
///      from the original tree-sitter kind), look it up in `PhpKind`,
...
pub fn transform(xot: &mut Xot, node: XotNode) -> Result<TransformAction, xot::Error> {
    ...
    let kind = match PhpKind::from_str(&kind_str) {
        Some(k) => k,
