...
### 3.4 Renderer-engine layering: what's shared across trees, and where it stops

There are four source-emitting engines in the codebase: SyntaxTree → source (8 languages through one unified path after S13), DataTree → JSON, DataTree → YAML, SqlTree → source. The question "how much engine code should they share?" comes up repeatedly. The answer is three discrete layers, taken in increasing order of risk and decreasing order of payoff. This section defines the layers so the labels used in TODO.md (S14-Z1 "Layer A", S14-Z4 "Layer B") have a stable referent.

**Layer A — shared leaf primitives.** ~100–200 LOC consolidation of utilities that have no walk semantics: `write_quoted_scalar(text, &QuoteStyle, escape_fn, out)` in `tree::render::common` is the canonical example. The same primitive is consumed by SyntaxTree's `String` arm, DataTree's `emit_string` (with JSON's escape table), DataTree-YAML's auto-promoted plain-or-double routing, and SqlTree's identifier emission. Plus `Indent`, `ByteRange.anchored`, the shared `QuoteStyle` enum itself. Risk: near zero — these are pure functions over leaf scalars, no per-variant walk to coordinate. Payoff: a change to one place propagates everywhere automatically. **Status: shipped under S14-Z1 / S14-Z2 / S14-Z3.**

**Layer B — shared orchestration shell, per-tree variant handlers.** A trait `SourceRenderer<T>` (or equivalent) with one method per tree that emits variant-specific tokens; a shared *driver* that owns:

- the source-anchored fast-path predicate (`tree_fully_anchored(t) && source.is_some() → byte-slice short-circuit`),
- the indent stack and output buffer,
- an *optional* span map output for splice-based mutation (`DataSpanMap` is built today by the DataTree renderers; SyntaxTree doesn't emit one yet but `tractor set` would benefit when structural mutation lands),
- the gap-fallback policy at anchored↔synthetic boundaries.

The variant-emit logic stays per-tree — each match arm encodes per-variant structural rules that genuinely don't compress further. The win is uniformity of *the parts around the variant arms*: one place to evolve gap-fallback policy, one entry-point shape `render(tree, source) -> (String, Option<SpanMap>)` for callers (mutation pipeline, format renderers, snapshot tools), one source of truth for the fast-path optimisation. Risk: medium — designing the trait shape correctly takes care, and migrating four engines to it is a real refactor. Payoff: structural mutation work (S15-Z3 onwards) gets span tracking on SyntaxTree without re-implementing the DataTree side; future trees plug in by implementing the trait. **Status: deferred (S14-Z4). Worth doing once a second concrete consumer of span tracking exists — today only `mutation/xpath_upsert.rs` reads them, and only for DataTree.**

**Layer C — single generic walker across all four engines.** A `Walker<T>` that consumes a `VariantHandler<T>` (closure table or reflection-style children-iterator) and drives the entire walk. This is the "monolithic unification" version. It sounds clean and falls apart in practice: typed-slot semantics (a `SyntaxTree::Class { generics, bases, body }` has *named* typed slots, not anonymous children) can't reduce to a generic walk without losing the variant clarity that makes each match arm readable. The walker either becomes a giant per-variant dispatch table (back to per-tree code with extra indirection) or loses the typed-slot meaning (can't render `class X<G> : B { ... }` from an iterator of unnamed children). **Status: rejected.** This is the same reason historic per-language transforms exist instead of a "universal CST normaliser" — the typed-slot vocabulary *is* the value.

**Decision rule.** Move primitives up the layers only when a concrete need pushes; never preemptively. Layer A is unconditionally worth doing (leaf utilities have no cost to share). Layer B waits for a second consumer of span tracking (or whatever the unifying need turns out to be). Layer C is not a destination — the per-variant walks are *supposed* to stay separate.

### 3.5 Examples — tree ↔ XML ↔ JSON
