...
| `tractor/src/matcher.rs:187` (`run_rules`) | Signature: `&[Source]` instead of `&[NormalizedPath]`. Per-rule glob match still on `&source.path`. Parsing reads via `source.read()` and pipes the bytes into the unified `parse(ParseInput::Inline { … }, ParseOptions { … })` entry. Parallel iteration preserved. |
