use std::io::Write;
use crate::output::{write_element_open_with_attrs, write_element_with_attrs_and_text, write_tag_close};

/// Write TreeSitter node to XML in raw format (original verbose output)
pub fn write_node(out: &mut impl Write, node: tree_sitter::Node, source: &str, indent: usize, use_color: bool) -> Result<(), Box<dyn std::error::Error>> {
    write_node_with_field(out, node, source, indent, use_color, None)
}

/// Write TreeSitter node with optional field name from parent
fn write_node_with_field(out: &mut impl Write, node: tree_sitter::Node, source: &str, indent: usize, use_color: bool, field_name: Option<&str>) -> Result<(), Box<dyn std::error::Error>> {
