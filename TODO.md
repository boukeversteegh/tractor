...
- [ ] [S16-Z24] **Dilemmas / requires-design-call.** Items where the pre-IR shape and the current shape are both defensible; flag here, decide before fixing:
  - **Python `pattern/pattern[union]` → `pattern[union]`.** Pre-IR had `pattern/pattern[union]/…`; current has `pattern[union]/…`. The pre-IR double-wrap arguably violates Principle #11 ("don't encode is-a hierarchy as wrapper elements"), so the flattening might be intentional. Confirm before restoring.
  - **Python `body[pass] = "pass"`.** Current emits text `"pass"` on the `body[pass]` marker; pre-IR had `body[pass]` empty. Adding text to a marker-only element is mildly off-pattern (Principle #13 — markers stay empty), but the text preserves source bytes for round-trip. Decide: empty marker (clean, regresses round-trip) vs textual marker (current, off-pattern).
  - **C#: `<value>/<expression>` double-wrap.** Pre-IR `value/expression/int="0"`; current still wraps in `<value>` but the inner `<expression>` is intermittent. May be intentional cleanup or a partial regression — needs per-variant audit.
