...
- [/] [XZ64] [S3B] **No `languages/<lang>/post_transform.rs` file exists; per-language shape decisions live in the lowering or in typed `Ir` variants; no global post-render pipeline survives either.**
  ...
  - [/] [S3B-Z1] **C#: lowering produces canonical IR end-to-end; `languages/csharp/post_transform.rs` does not exist. (Pathfinder for the per-language fold + shared-helper consolidation pattern.)**
    ...
    - [ ] [S3B-Z1b] **Shared cross-language post-render helpers run from a single registry-driven pipeline, with their per-language data lists carried as `LanguageOps` fields. No language declares its own copy of `tag_multi_role_children`-style invocations.** *(Sibling concern — not C#-specific. Unblocks Z1c and Z2…Z9's file-deletion steps.)*
      - New `LanguageOps` fields for the data each shared helper needs: `list_tags: &'static [(&'static str, &'static str)]`, `tag_multi_same_name_kinds: &'static [&'static str]`, `expression_position_slots: &'static [&'static str]`, `body_strip_names: &'static [&'static str]`, `single_declarator_parents: &'static [&'static str]`, `distribute_member_list_parents: &'static [&'static str]`, etc.
      - Populate each row in `LANGUAGES` with its current data (extract from each language's `post_transform.rs` body).
      - Replace per-language `post_transform` fns with a single `apply_post_render_passes(xot, root, ops)` invoked once after `render_to_xot` (or fold into `to_xot` itself where the helper's data shape allows).
      - Coordinate with **S3D** (drop `list="X"` attribute pass once `to_xot` reads cardinality from typed IR slots) — once S3D lands, the `tag_*` helpers shrink or disappear and the `LanguageOps` data tables shrink with them. Don't lock the field shape until S3D's direction is decided.
      - After: every per-language `post_transform.rs` is reduced to its language-specific content, which then migrates language-by-language via Z2…Z9.

    - [ ] [S3B-Z1c] **`languages/csharp/post_transform.rs` does not exist; csharp's `LANGUAGES` row has `post_transform: None`.**
      - Depends on Z1a (no C#-specific work left in the file) + Z1b (shared scaffolding migrated out).
