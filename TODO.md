...
2. ~~**2️⃣ S16** — Address the 11 pre-existing snapshot regressions.~~ — Z1/Z2/Z3/Z4/Z7 landed 2026-05-16; Z5/Z8/Z9 closed as addressed by S17; Z6 deferred (architectural tension); Z24 dilemmas resolved per user calls. Snapshots regenerated cleanly. Remaining work tracked under Z26 (C# value/expression audit) and Z27 (residual S17 shape regressions on closed Z16/Z21/Z22 contracts).
...
- [ ] [S16-Z6] **Top-level field grouping (`imports: [...]`, `namespaces: [...]`)** — **architectural tension; deferred**. The pre-S16 `data_to_json` projection had per-language curated grouping; S17 retired `data_to_json` and routes JSON directly from `SyntaxTree` via a variant-blind walker. Restoring grouping would mean re-introducing per-language render-time logic — a direct violation of S17's "no variant knowledge in the renderer" principle. **Options:** (a) accept the new shape (`{$type: "file", $children: [...], …}` with children in source order) as canonical, (b) introduce a post-pass on the rendered JSON to group by `$type` key (compatible with variant-blind walker since it operates on output, not tree), (c) revive a lighter `data_to_json` for the grouped projection only and document as a separate output mode. Needs design call before implementing — flagged 2026-05-16.
