...
- [ ] [S16-Z27] **Residual S17-induced shape regressions on previously-closed Z items.** Surfaced 2026-05-16. Closed two items 2026-05-16 via `@element_name` annotations (`Z16` `TypeParameter` → `<type>`, `Z11`-related `GenericType` → `<type>`); the remaining items need variant restructuring rather than just an element-name override:
  - ~~**Z16 (Python PEP 695 `<generic>/<type>/<name>`)**~~ ✅ closed 2026-05-16. `TypeParameter` now carries `@element_name = element_name_for_type_parameter` returning `"type"`; the rest of the shape was already correct.
  ...
  - **Z22 (Ruby heredoc opener as `<string>` value).** Test `ruby_heredoc_opener_lowers_to_string_value` — runtime contract that the heredoc opener `<<-TEXT` survives as `right/expression/string="<<-TEXT"`. Same root cause as Z21 (the right-hand side isn't getting the `<right>` wrapper) plus the opener-text encoding.
  - **Other slot-wrapper regressions (newly visible 2026-05-16).** Fixing `render_to_xot` test compile errors exposed three more contracts that haven't been re-landed since S17: `ir_csharp_parity::cast_expression` (`<value>` slot around cast operand — Z26 family), `ir_csharp_parity::conditional_access_isomorphism` (`<member>` slot for `AccessSegment::Member` in the variant-blind walker), `ir_csharp_parity::is_type_test` (typed-slot wrappers around the is-test target), and `ir_python_blueprint::blueprint_coverage_audit` (variant-blind walker emits stray gap-text after an `If`'s nested `else_branch`). All four are `#[ignore]`d with a Z27 pointer.
...
- [x] [S16-Z28] **Restore conventional element names on typed access / generic / raw variants** ✅ — landed 2026-05-16. Three typed variants were renamed for clarity but lost their conventional XPath element names under S17's snake_case-of-variant default; restored via the same `@element_name` annotation mechanism as Z16/Z11:
  - `SyntaxTree::Access` → renamed to `SyntaxTree::ObjectAccess`; `@element_name` returns `"object"` so `//object[name='console']` queries keep working. The variant name disambiguates from the `Access` visibility enum and the `AccessReceiver` kind. (User call: "just object in all languages that use it.")
  - `SyntaxTree::Raw` → `@element_name` returns the tree-sitter `kind` (`<let_declaration>`, `<object>`, `<pair>`, ...) via one-time `Box::leak` per distinct kind. Raw passthrough lowering also rewired: anonymous tree-sitter tokens lower to `SyntaxTree::Inline` (no wrapper) instead of `Raw { is_named: false }`, so the override is total — every `Raw` has a real kind.
  - `SyntaxTree::GenericType` → `@element_name` returns `"type"` so `//type[.='List<int>']` queries match non-parameterized type references uniformly.
  - Codegen also picked up `Option<&'static str>` marker fields named `marker`: `SyntaxTree::Expression.marker` now emits its inner string as a `<{name}/>` child, restoring `<expression[non_null]>` etc. for C# null-forgiving operators (Principle #15).
  - CLI test fixtures updated: `//object[access and X]` predicates dropped the stale `[access]` marker check (`//object[X]`) since the typed `<object>` no longer carries the discriminator marker.
